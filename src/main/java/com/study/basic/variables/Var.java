package main.java.com.study.basic.variables;

public class Var {

    public static void main(String[] args) {

        /*
        int a = "a";

        System.out.println("a = " + a);
         */

        byte b = 127; // -128 ~ 127
        short s = 32767; // -32,768 ~ 32,767
        int i = 2147483647; // -2,147,483,648 ~ 2,147,483,547 (약 20억)
        long l = 9223372036854775807L; // -9,223,372,036,854,775,808 ~ 9,223,372,036,854,775,807

        float f = 10.0f;
        double d = 10.0;

        System.out.println("l = " + l);
        System.out.println("d = " + d);

        int exNum1 = 4;
        int exNum2 = 3;

        System.out.println(exNum1 + exNum2);
        System.out.println(exNum1 - exNum2);
        System.out.println(exNum1 * exNum2);

        int exSum = exNum1 + exNum2;

        System.out.println(exSum);

        long exLong = 10000000000L;
        System.out.println("exLong = " + exLong);

        boolean exBool = true;
        System.out.println("exBool = " + exBool);
    }
}
